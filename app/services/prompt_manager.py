...
class PromptManager:
    ...
    def generate_wrapper_prompt(
        ...
        wrapper_template = self.get_wrapper_template(
            source_type, indicator_metadata.periodicity
        )
    ...
    def generate_wrapper_lint_retry_prompt(
        self, generated_code: str, linting_errors: str
    ) -> str:
        """Generate retry prompt for wrapper regeneration after critical linting failures"""
        template = self.env.get_template("wrapper_lint_retry.j2")
        return template.render(
