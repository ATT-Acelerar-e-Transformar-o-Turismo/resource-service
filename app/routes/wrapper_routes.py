    ...
    try:
        ...
        if request.source_type in [SourceType.CSV, SourceType.XLSX]:
            if isinstance(request.source_config, (CSVSourceConfig, XLSXSourceConfig)):
                uploaded_file = await file_service.get_uploaded_file(
                    request.source_config.file_id
                )
                if not uploaded_file:
                    raise HTTPException(
                        status_code=400,
                        detail=f"File not found: {request.source_config.file_id}",
                    )
                # Populate location with computed file path
                request.source_config.location = uploaded_file.file_path
                logger.info(
                    f"Resolved file_id {request.source_config.file_id} to path: {uploaded_file.file_path}"
                )
...
async def get_wrapper_logs(wrapper_id: str, limit: int = 200) -> dict:
    ...
    try:
        ...
        return {"wrapper_id": wrapper_id, "logs": logs, "total_lines": len(logs)}
    except HTTPException:
        raise
